...
class IVON(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        self._device = device if not device == "" else self._device
        self.current_step = 0
        self.debias = debias
    ...
    def hess_stats(self):
        ...
        for group in self.param_groups:
            hess_stats.append(
                {
                    "hess_mean": group["hess"].mean().item(),
                    "hess_std": group["hess"].std().item(),
                    "hess_min": group["hess"].min().item(),
                    "hess_median": group["hess"].median().item(),
                    "hess_max": group["hess"].max().item(),
                }
            )
        return hess_stats
