...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.CLORA, PeftType.ADALORA, PeftType.VILORA):
        ...
        if getattr(model, "compute_hessian", False):
            to_return = {k: v for k, v in to_return.items() if "hessian" in k}
        else:
            to_return = {k: v for k, v in to_return.items() if (("lora_" in k and adapter_name in k) or ("bias" in k))}
        if config.peft_type in [PeftType.ADALORA, PeftType.VILORA]:
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        parameter_prefix = {
            PeftType.IA3: "ia3_",
            PeftType.LORA: "lora_",
            PeftType.CLORA: "lora_",
            PeftType.ADALORA: "lora_",
            PeftType.VILORA: "lora_",
            PeftType.LOHA: "hada_",
            PeftType.LOKR: "lokr_",
            PeftType.OFT: "oft_",
            PeftType.POLY: "poly_",
        ...
        for k, v in state_dict.items():
            if parameter_prefix in k:
                ...
                if "." in suffix:
                    ...
                    k = f"{k}.{adapter_name}" if not "hessian" in k else k
