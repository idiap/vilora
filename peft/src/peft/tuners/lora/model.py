...
class CLoraModel(LoraModel):
    def __init__(self, model, config, adapter_name):
        ...
        self.hessian_method = config[adapter_name].hessian_method

        if self.hessian_method != 'none':
            self.init_hessian_buffer()

    def init_hessian_buffer(self):
        self.modules_to_adapt = {}
        peft_config = self.peft_config[self.active_adapter]
        for k, v in self.model.named_modules():
            if self._check_target_module_exists(peft_config, k):
                self.modules_to_adapt[k] = v
                ...
                if self.hessian_method in ['ewc', 'rewc', 'aewc']:
                ...
                elif self.hessian_method in ['kfac', 'ekfac']:
                ...
                elif self.hessian_method == 'tkfac':
    ...
    def prepare_compute_hessian(self, ewc_ver='v1'):
        self.compute_hessian = True
        self.backward_hook, self.forward_hook = None, None
        self.backward_hook_handles, self.forward_hook_handles = {}, {}
        self.disable_adapter_layers()

        if self.hessian_method in ['ewc', 'rewc', 'aewc']:
            assert ewc_ver in ['v1', 'v2']
            if ewc_ver == 'v1':
                self.set_hook_ewc_v1()
            elif ewc_ver == 'v2':
                self.set_hook_ewc_v2()
        elif self.hessian_method in ['kfac', 'ekfac']:
            self.set_hook_kfac()
        elif self.hessian_method == 'tkfac':
            self.set_hook_tkfac()

        for k, v in self.modules_to_adapt.items():
            v.weight.requires_grad_(True)
            if self.backward_hook:
                self.backward_hook_handles[k] = v.register_full_backward_hook(self.backward_hook)
            if self.forward_hook:
                self.forward_hook_handles[k] = v.register_forward_hook(self.forward_hook)

    def compute_hessian_requires_grad(self):
        for k, v in self.modules_to_adapt.items():
            v.weight.requires_grad_(True)

    def set_hook_ewc_v1(self):
        ...
        if self.hessian_method in ['ewc', 'rewc']:
            self.backward_hook = backward_hook_ewc
        elif self.hessian_method == 'aewc':
            self.backward_hook = backward_hook_aewc

    def set_hook_ewc_v2(self):
        def backward_hook_ewc(module, grad_input, grad_output):
        ...
        if self.hessian_method in ['ewc', 'rewc']:
            ...
            self.backward_hook = backward_hook_aewc
        self.forward_hook = forward_hook_ewc

    def set_hook_kfac(self):
        ...
        self.backward_hook = backward_hook_kfac
        self.forward_hook = forward_hook_kfac
    
    def set_hook_tkfac(self):
        ...
        self.backward_hook = backward_hook_tkfac
        self.forward_hook = forward_hook_tkfac

    def finalize_hessian(self, num_samples):
        self.remove_hooks()

        for k, v in self.modules_to_adapt.items():
            if self.hessian_method in ['ewc', 'rewc', 'aewc']:
                v.lora_hessian_ewc /= num_samples
                if hasattr(v, 'lora_temp_ewc_x'):
                    del v.lora_temp_ewc_x
            elif self.hessian_method in ['kfac', 'ekfac']:
                v.lora_hessian_kfac_G /= num_samples
                v.lora_hessian_kfac_A /= num_samples
            elif self.hessian_method == 'tkfac':
                v.lora_hessian_tkfac_Psi /= num_samples
                v.lora_hessian_tkfac_Phi /= num_samples
                v.lora_hessian_tkfac_delta /= num_samples
    ...
    def remove_hooks(self):
        for k, handle in self.backward_hook_handles.items():
            handle.remove()
        for k, handle in self.forward_hook_handles.items():
            handle.remove()

    def compute_hessian_loss(self):
        loss = 0
        if self.hessian_method == 'l2sp':
            for k, v in self.modules_to_adapt.items():
                loss += torch.sum(v.get_delta_weight(self.active_adapter) ** 2)
        elif self.hessian_method in ['ewc', 'rewc']:
            for k, v in self.modules_to_adapt.items():
                loss += torch.sum(v.lora_hessian_ewc * v.get_delta_weight(self.active_adapter) ** 2)
            if self.hessian_method == 'rewc':
                loss = (loss + 1e-8) ** 0.5
        elif self.hessian_method == 'aewc':
            for k, v in self.modules_to_adapt.items():
                loss += torch.sum(v.lora_hessian_ewc * torch.abs(v.get_delta_weight(self.active_adapter)))
        elif self.hessian_method == 'kfac':
            for k, v in self.modules_to_adapt.items():
                delta_weight = v.get_delta_weight(self.active_adapter)
                delta_weight = delta_weight if not v.fan_in_fan_out else delta_weight.T
                G = v.lora_hessian_kfac_G
                A = v.lora_hessian_kfac_A
                loss += torch.sum(delta_weight * (G @ delta_weight @ A))
        elif self.hessian_method == 'ekfac':
            self.compute_ekfac()
            for k, v in self.modules_to_adapt.items():
                ...
                loss += torch.dot(v_kfe.view(-1)**2, diag.view(-1))
        elif self.hessian_method == 'tkfac':
            for k, v in self.modules_to_adapt.items():
                delta_weight = v.get_delta_weight(self.active_adapter)
                delta_weight = delta_weight if not v.fan_in_fan_out else delta_weight.T
                ...
                loss += delta * torch.sum(delta_weight * (Psi @ delta_weight @ Phi))
        return loss

    def compute_ekfac(self):
        if getattr(self, 'ekfac_init', False):
            return
