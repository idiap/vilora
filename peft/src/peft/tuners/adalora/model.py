...
from .layer import AdaLoraLayer, RankAllocator, VIRankAllocator, SVDLinear
...
class VILoraModel(AdaLoraModel):
    def __init__(self, model, config, adapter_name):
        super().__init__(model, config, adapter_name)
        ...
        if self.peft_config[adapter_name].inference_mode:
            ...
            self.trainable_adapter_name = adapter_name
            self.rankallocator = VIRankAllocator(self.model, self.peft_config[adapter_name], self.trainable_adapter_name)

    def set_optimizer(self, optimizer):
        if hasattr(self, "rankallocator"):
            setattr(self.rankallocator, "optimizer", optimizer)
