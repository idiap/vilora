...
class VIRankAllocator(RankAllocator):
    def __init__(self, model, peft_config, adapter_name):
        super().__init__(model, peft_config, adapter_name)
        self.criterion = peft_config.criterion
    ...
    def score_SNR(p, p_sigma):
        return torch.abs(p) / p_sigma
    
    @staticmethod
    def score_E_mean_abs(p, p_sigma):
        return p * torch.erf(p / p_sigma / np.sqrt(2)) + 2 * p_sigma * torch.exp(-p**2 / (2 * p_sigma**2)) / np.sqrt(2 * np.pi)
    
    @staticmethod
    def score_SNR_mean_abs(p, p_sigma):
        score = p * torch.erf(p / p_sigma / np.sqrt(2)) + 2 * p_sigma * torch.exp(-p**2 / (2 * p_sigma**2)) / np.sqrt(2 * np.pi)
        return score / torch.sqrt(p_sigma**2 + p**2 - score**2)
    ...
    def update_and_allocate(self, model, global_step, force_mask=False):
        # # Update the importance score and allocate the budget
        if self.peft_config.tinit < global_step <= self.peft_config.total_step - self.peft_config.tfinal and global_step % self.peft_config.deltaT == 0:
            self.update_ipt(model)
        budget, mask_ind = self.budget_schedule(global_step)
        # Allocate the budget according to importance scores
        if mask_ind or force_mask:
